...
public class HibernateTest {
	...
	public static void main(String[] args) {
		...
		// Persisting a detached object
		 session = factory.openSession();
		 session.beginTransaction();
		 // this will trigger an update query to the db, even if there is no change in the object's state	
		 session.update(user);
		 // Once update is done, now the object is in persisted state
		 user.setUserName("Ajay After update");
		 session.getTransaction().commit();
		 session.close();
	}
}
